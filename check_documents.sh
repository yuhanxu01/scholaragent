...
echo "=== Checking documents and their owners ==="
curl -H "Authorization: Bearer $NEW_TOKEN" "http://localhost:8000/api/documents/?page_size=3" | jq '.results[] | {title, user: .user, collections_count, is_collected}'